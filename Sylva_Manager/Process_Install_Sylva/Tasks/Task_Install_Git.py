...
for t in p:
	output = output + str( t.decode('utf-8'))+" \r\n"
p = subprocess.Popen(["ssh  root@"+ip+" apt install git-all "], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
for t in p:
	output = output + str( t.decode('utf-8'))+" \r\n"
...
ret = MSA_API.process_content('FAILED', 'Task OK '+ output, context, True)
print(ret)
