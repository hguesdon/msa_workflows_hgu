...
response2 = requests.request("POST", url='https://eu01.sb.infonova.com/r6-api/SB_CATALYST_AWS_US/serviceordering/v1/notification', headers=_headers2, data=json.dumps(data2), verify=False)
r_json2 = response2.json()
context.update(infonova = dict(response=r_json2,message=r_json2['messages'][0]['message'],satus_code=response2.status_code))

MSA_API.task_success('Infonova Order state Change Notification for order sent\n' +r_json2['messages'][0]['message']+'\n Status Code : '+str(response2.status_code)+'', context, True)